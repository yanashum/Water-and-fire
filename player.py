...
class Player:
    ...
    def move(self, dx, dy, grid, friends, power_ups):
    ...
    def plant_water(self, map):
        b = water(self.range, round(self.pos_x / Player.TILE_SIZE), round(self.pos_y / Player.TILE_SIZE), map, self)
        return b
    ...
    def consume_power_up(self, power_up, power_ups):
        if power_up.type == PowerUpType.water:
            self.barrel_limit += 1
